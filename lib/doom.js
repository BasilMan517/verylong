...
export function analyzeAnswers(answerIds) {
  ...
  const comboPenalty =
    (scores.stress >= 5 && scores.sleep >= 4 ? 4 : 0) +
    (scores.metabolic >= 5 && scores.gut >= 3 ? 3 : 0) +
    (scores.cardiovascular >= 3 && scores.sleep >= 3 ? 3 : 0) +
    (scores.metabolic >= 4 && tags.includes("社交性饮酒") ? 3 : 0) +
    (scores.stress >= 4 && tags.includes("酒精安眠药") ? 3 : 0) +
    (scores.mood >= 4 && scores.sleep >= 4 ? 2 : 0);

  const controlBonus = Math.max(scores.control, 0) * 2;
  const baseExpectancy = userAge < 30 ? 89 : userAge < 45 ? 84 : 79;
  const estimatedAge = Math.max(
    userAge + 3,
    Math.min(99, Math.round(baseExpectancy - weightedRisk * 1.1 - comboPenalty + controlBonus)),
  );
...
function buildReport({ scores, tags, archetype, cause, estimatedAge, userAge }) {
  ...
  const foreshadowBank = {
    stress: [
      "你的心率已经学会根据钉钉通知自动调整档位，比手环算法还灵敏。皮质醇浓度已经高到可以申请危险化学品许可证。",
      "压力不是用来管理的，是用来假装不存在的——你的肾上腺已经接受了这个事实，并开始默默写辞职信。",
      "你的身体把「休息一下」理解为「换个屏幕继续看」，神经系统对此表示了持续性抗议。",
    ],
    sleep: [
      "你的生物钟已经不是倒时差的问题，而是彻底放弃了对时区的信仰。褪黑素分泌系统已申请调岗至白班。",
      "凌晨两点，你的大脑还在召开没有议程的全体会议。睡眠债的利率已经超过了花呗。",
      "你以为年轻可以把睡眠当信用卡透支，但身体这家银行从不减免滞纳金。",
    ],
    metabolic: [
      "你的胰岛素分泌团队已经连续加班三千多天，正在讨论集体维权的可能性。血糖曲线像极了过山车——但从不卖票。",
      "外卖平台的推荐算法比你的身体更了解你需要什么。不幸的是，它的目标函数里没有「活久一点」。",
      "你的代谢系统正在处理一份永远也结不了案的外卖残留案件，证据已经从胃积压到了血管。",
    ],
    cardiovascular: [
      "你的血管内壁正在进行一场缓慢的装修工程——材料是胆固醇，工期是余生。心脏表示工作量已超过劳动法上限。",
      "椅子已经记住了你的体型，但你的心肺系统正在忘记全速运转是什么感觉。楼梯成了生死测验。",
      "祖传的血压基因加上后天的久坐buff，你的心血管系统正在运行一场没有补丁的老版本程序。",
    ],
    gut: [
      "你的肠胃已经从消化器官升级为情绪表达器官——每次加班到深夜，它都会用疼痛发表评论。",
      "胃酸和夜宵之间的谈判已经破裂多次。肠道菌群正在考虑更换宿主。",
      "你的消化系统像一个被迫7×24营业的便利店，货架乱了，排风扇坏了，店员已经在写离职信。",
    ],
    mood: [
      "你的情绪管理方案是「先忍着」，备用方案是「继续忍着」。大脑的快乐激素库存已经见底，正在从隔壁焦虑部门借调。",
      "精神内耗的效率已经超过了任何实际工作。你的多巴胺账户余额不足，但系统拒绝充值。",
      "你把社交需求外包给了屏幕，但屏幕只收费不提供售后。孤独正以每天15根烟的速度计费。",
    ],
    inflammation: [
      "体检报告上的小箭头已经集齐了一整支乐队，正在排练一首叫《慢性炎症进行曲》的曲子。",
      "你的免疫系统分不清敌我，正在对自己的组织发起一场旷日持久的内战。",
      "那些被你忽略的指标偏高不是错别字，是身体在用荧光笔做标记。",
    ],
  };
